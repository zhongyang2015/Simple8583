			...
			if (field.isAppData()) {//数据域
				if (hasBitMap) {
					...
					if (bitMap.getBit(index - 1) == 1) {//XXX 位图中是否存在值
						//针对于密钥重置和签到两个接口的时候63域需要特殊处理
						if("000000".equals(returnMap.get("3")) || "000010".equals(returnMap.get("3"))){
							//因为有的63域为报文DES加密域所以特殊处理
